...
const Dashboard = () => {
  ...
  const toggleOpenIndex = (index: number | null) => {
    let value = index
    if (openIndex === index) {
      value = null
    }
  ...
  const renderScoreInputs = () => {
    ...
    return (
      <div className="space-y-6">
        ...
        <div className={`card border rounded bg-white ${openIndex === 1 ? 'active' : ''}`}>
          <h3 className="card_header p-4 font-bold text-sm md:text-base cursor-pointer" onClick={() => toggleOpenIndex(1)}>
            Dấu hiệu nhận diện thương hiệu (20 điểm)
          </h3>
          <div className={`card_content overflow-hidden transition-all duration-300 ${openIndex !== 1 ? 'max-h-0' : 'max-h-screen'}`}>
  ...
  return (
    <div className="bg-gray-100 max-w-full min-h-screen">
      <style>{rangeStyles}</style>
      <h1 className="text-gray-900 text-6xl dark:text-white text-center py-10 font-black">
        CHẤM ĐIỂM
      </h1>c
